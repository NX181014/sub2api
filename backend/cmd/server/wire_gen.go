...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	poolApprovalRepository := repository.NewPoolApprovalRepository(client)
	poolService := service.NewPoolService(poolRepository, poolApprovalRepository, adminService, client, settingService)
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, poolService)
	...
	poolHandler := admin.NewPoolHandler(poolService)
	mihomoRepository := repository.NewMihomoRepository(db)
	mihomoService := service.ProvideMihomoService(configConfig, proxyRepository, secretEncryptor, mihomoRepository, proxyExitInfoProber)
	mihomoHandler := admin.NewMihomoHandler(mihomoService, poolService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	ollamaCloudUsageService := service.ProvideOllamaCloudUsageService(accountRepository, httpUpstream, settingService, secretEncryptor, configConfig, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, poolHandler, mihomoHandler, poolService, mihomoService, upstreamBillingProbeService, ollamaCloudUsageService)
